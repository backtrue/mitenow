...
import { validateSessionAndGetUser } from '../utils/session-manager';
...
export async function handleDeploy(
  ...
  // Get user ID from session (optional - anonymous users allowed)
  ...
  try {
    ...
    if (sessionMatch) {
      ...
      const sessionResult = await validateSessionAndGetUser(env, sessionId);
      if (sessionResult) {
        userId = sessionResult.user.id;
      }
    }
  } catch {
    // Anonymous deployment - user_id stays null
  }
  ...
  try {
    ...
    // Create deployment record in D1 for dashboard display
